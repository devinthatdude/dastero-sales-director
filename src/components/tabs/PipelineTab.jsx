...
export default function PipelineTab({ leads, tags, updateLead, onOpen }){
  ...
  return (
    <div className="px-4 pt-5">
      <div className="text-[11px] uppercase tracking-widest font-semibold" style={{color:'#2F6BF0'}}>Sales Pipeline</div>
      <h1 className="text-3xl font-bold mt-0.5 mb-1">Pipeline</h1>
      <div className="soft text-sm mb-4">{active.length} active · <span className="mono">{money(openVal)}</span> in play</div>
        ...
        {STAGES.map(st=>{
          ...
          return (
            <div key={st.id}
              ...
              {items.length===0
                ? <div className="text-xs dim text-center py-4 rounded-lg" style={{border:'1px dashed #DCE4F1'}}>—</div>
                : items.map(l=>(
      ...
      {moving && (
        <div className="fixed inset-0 z-50 flex items-end justify-center"
          style={{background:'rgba(4,7,15,.5)'}} onClick={()=>setMoving(null)}>
          <div className="panel w-full max-w-lg rounded-t-3xl p-5 pb-8" onClick={e=>e.stopPropagation()}>
            <div className="text-[11px] uppercase tracking-widest font-semibold mb-1" style={{color:'#2F6BF0'}}>Move stage</div>
            <div className="font-bold text-lg mb-4">{moving.company}</div>
              ...
              {STAGES.map(s=>(
                <button key={s.id} onClick={()=>{ updateLead(moving.id,{stage:s.id}); setMoving(null); }}
                  ...
                  style={{
                    ...
                    border: `1px solid ${moving.stage===s.id ? s.color : '#DCE4F1'}`,
                  }}>
                  <span className="w-2.5 h-2.5 rounded-full flex-none" style={{background:s.color}}/>
                  <span className="font-semibold text-sm" style={{color: moving.stage===s.id ? s.color : '#5C6B85'}}>{s.name}</span>
                  {moving.stage===s.id && <span className="ml-auto text-xs" style={{color:s.color}}>current</span>}
                </button>
              ))}
            </div>
            <button onClick={()=>setMoving(null)} className="mt-4 w-full text-sm font-semibold py-2.5 rounded-xl dim" style={{border:'1px solid #DCE4F1'}}>Cancel</button>
          </div>
        </div>
      )}
    </div>
