...
function Bar({label,value,max,color}){
  ...
  return (
    <div className="flex items-center gap-3 py-1.5">
      <span className="text-sm w-36 truncate">{label}</span>
      <div className="flex-1 h-2 rounded-full" style={{background:'#DCE4F1'}}>
        <div className="h-2 rounded-full" style={{width:pct+'%',background:color}}/>
      </div>
      <span className="mono text-sm font-semibold w-6 text-right">{value}</span>
    </div>
  );
}
...
export default function StatsTab({ leads, tags, profiles=[], isAdmin }){
  ...
  return (
    <div className="px-4 pt-5">
      <div className="text-[11px] uppercase tracking-widest font-semibold" style={{color:'#2F6BF0'}}>Performance</div>
      <h1 className="text-3xl font-bold mt-0.5 mb-4">Stats</h1>

      <div className="grid grid-cols-2 gap-2.5 mb-4">
        <div className="surface rounded-2xl p-4"><div className="dim text-[11px] uppercase font-semibold">Pipeline</div><div className="mono text-2xl font-bold mt-1">{money(pipelineVal)}</div></div>
        <div className="surface rounded-2xl p-4"><div className="dim text-[11px] uppercase font-semibold">Closed Won</div><div className="mono text-2xl font-bold mt-1" style={{color:'#1B9E6E'}}>{money(wonVal)}</div></div>
      </div>
        ...
        <Section title="Rep Leaderboard">
          {repStats.map((r,i)=>(
            <div key={r.name} className="flex items-center gap-3 py-2">
              ...
              <span className="text-sm font-semibold w-28 truncate">{r.name}</span>
              <div className="flex-1 h-2 rounded-full" style={{background:'#DCE4F1'}}>
                <div className="h-2 rounded-full" style={{width:Math.round(r.pipeline/maxPipeline*100)+'%',background:'#2F6BF0'}}/>
              </div>
              <div className="text-right w-20 flex-none">
                <div className="mono text-xs font-semibold">{money(r.pipeline)}</div>
      ...
      <Section title="Lead Sources">{Object.keys(sources).length? Object.entries(sources).map(([k,v])=> <Bar key={k} label={k} value={v} max={maxSrc} color="#1B9E6E"/>):<Empty/>}</Section>
      <Section title="Services in Play">{Object.keys(services).length? Object.entries(services).map(([k,v])=> <Bar key={k} label={k} value={v} max={maxSvc} color="#2F6BF0"/>):<Empty/>}</Section>
      <Section title="Tags Breakdown">{Object.keys(tagCounts).length? Object.entries(tagCounts).map(([id,v])=>{const t=tags.find(x=>x.id===id);return <Bar key={id} label={t?`${t.emoji||''} ${t.label}`:'—'} value={v} max={maxTag} color={t?t.color:'#92A0B8'}/>;}):<Empty/>}</Section>
    </div>
  );
}
